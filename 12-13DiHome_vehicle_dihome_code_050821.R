...
pheno_data <- read.table("/Users/kmagnaye/Documents/Lynch_Lab/12-13DiHOME_macs/dihome_human/pheno_data.txt", stringsAsFactors = FALSE,  header = TRUE)
treatment_allergen <- as.factor(paste0(pheno_data$Treatment, "_", pheno_data$Allergen))
...
pheno_data$Treatment_Allergen <- as.factor(paste0(pheno_data$Treatment, "_", pheno_data$Allergen))
...
## PCA plot by treatment and by timepoint, separately

human_pcs12 <- t(sum.PC$x[,1:2])
human_pcs12_final <- human_pcs12[,1:18]
pheno_data_final <- pheno_data[1:18,]  

ggplot(pheno_data_final, aes(x=human_pcs12_final[1,], y=human_pcs12_final[2,], color=Treatment_Allergen)) +
  geom_point() + theme_bw() + stat_ellipse(aes(x=human_pcs12_final[1,], y=human_pcs12_final[2,],color=Treatment_Allergen, group=Treatment_Allergen),type = "norm") + xlab("PC1 (26.8%)") + ylab("PC2 (18.6%)")

lme_pc1_treatment_allergen <- anova(lmer(as.numeric(human_pcs12_final[1,]) ~ as.factor(Treatment_Allergen) + (1|Patient), data = pheno_data_final)) # P=0.01
lme_pc2_treatment_allergen <- anova(lmer(as.numeric(human_pcs12_final[2,]) ~ as.factor(Treatment_Allergen) + (1|Patient), data = pheno_data_final)) # P=2.1x10-4

  apply(voom_norm_nomedia_dihomepnut_dmso, 1, function(x) summary(lmer(as.numeric(x) ~ as.factor(treatment_allergen) + as.factor(Timepoint) + (1|Patient), data = pheno_data_nomedia_dihomepnut_dmso))$coefficients[2,1])
